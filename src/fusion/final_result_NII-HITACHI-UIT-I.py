...
def main():
    ...
    # topic del
    topics_file = '/net/dl380g7a/export/ddn11c1/wangz/ins2018/del/topics_I.npy'
    if os.path.isfile(topics_file):
        topic_del = np.load(topics_file)
    else:
        topic_del = np.zeros((gallery_num, 30), dtype=int)
        del_xls_file_path = '/net/dl380g7a/export/ddn11c1/wangz/ins2018/del/del_I.xlsx'
    ...
    for i in range(30):
        ...
        del_result = np.copy(topic_del[:, i])
        del_result[del_result == 0] = -1
        del_result[del_result > 0] = 0
        del_result[del_result == -1] = 1.0
        final_similarity_total[i, :] = final_similarity_total[i, :] * del_result

        final_similarity_threshold_list  = [200,  400,  600,  800,  1000, 3000]
        scene_similarity_threshold_list  = [500,  1000, 2000, 3000, 4000, 10000]
        person_similarity_threshold_list = [1000, 2000, 3000, 4000, 6000, 15000]
        ...
        for k in range(6):
            for j in range(gallery_num):
                if final_similarity_total[i, j] > 0:  # adjust by the scores in neighborhood
                    ...
                    if temp_times > 3 & temp_temp_scene > 2 & temp_temp_person > 2:
    ...
    # noface
    noface_file = '/net/dl380g7a/export/ddn11c1/wangz/ins2018/del/noface.npy'
    if os.path.isfile(noface_file):
        noface = np.load(noface_file)
    else:
        noface = np.zeros(gallery_num, dtype=int)
        noface_mat_data_file = '/net/dl380g7a/export/ddn11c1/wangz/ins2018/del/noface_2016.mat'
        mat_data = scio.loadmat(noface_mat_data_file)
        noface_list = mat_data['noface']
        cell_num = len(noface_list)
        for i in range(cell_num):
            shot_id = noface_list[i][0][0]
            index_a = shot_id.find('shot')
            index_b = shot_id.find('_')
            shot_id_num_1 = shot_id[index_a + 4:index_b]
            shot_id_num_2 = shot_id[index_b + 1:]
            shot_position = list(set(np.where(shot_index[:, 0] == int(shot_id_num_1))[0]).intersection(
                set(np.where(shot_index[:, 1] == int(shot_id_num_2))[0])))[0]
            noface[shot_position] = 1
        np.save(noface_file, noface)
    print('outdoor scene loaded')
    ...
    for i in range(topics.shape[0]):
        ...
        del_result = shot0_shots + del_scene + topic_del[:, i] # + noface
        if topics[i, 1] != pub:
        ...
        for k in range(100):
            final_similarity[final_similarity_result[10*k:10*(1+k)]] = 100 - k + final_similarity[final_similarity_result[10*k:10*(1+k)]] + 20*person_similarity[final_similarity_result[10*k:10*(1+k)], topics[i, 1]]
